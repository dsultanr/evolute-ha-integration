...
async def async_setup_entry(
    ...
    """Set up Evolute binary sensors based on a config entry."""
    ...
    for car in coordinator.cars:
        for sensor_key, (name, device_class, state_key) in BINARY_SENSOR_TYPES.items():
            entities.append(
                EvoluteBinarySensor(coordinator, car, sensor_key, name, device_class, state_key)
            )

    async_add_entities(entities)


class EvoluteBinarySensor(CoordinatorEntity, BinarySensorEntity):
    """Representation of an Evolute binary sensor."""
    ...
    def is_on(self) -> bool:
        ...
        return bool(car_data.get(self._state_key))
